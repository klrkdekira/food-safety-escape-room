...
function checkPuzzle(id) { const puzzle = Game.puzzleData[id]; if (!puzzle || Game.state.puzzleSolved[id]) return; Game.state.puzzleAttempts[id]++; let correct = false; if (puzzle.type === 'mcq') correct = selectedMCQ[id] === puzzle.correct; else if (puzzle.type === 'order') { const order = Array.from(document.querySelectorAll(`#puzzle-${id}-list .order-item`)).map(i => i.dataset.id); correct = JSON.stringify(order) === JSON.stringify(puzzle.correctOrder); } else if (puzzle.type === 'multiselect') { const sel = Array.from(selectedMulti[id] ?? []).sort(); correct = JSON.stringify(sel) === JSON.stringify([...puzzle.correct].sort()); } const rEl = document.getElementById(`result-${id}`); const pEl = document.getElementById(`p${id}`); if (correct) { const pts = Game.state.puzzleAttempts[id] === 1 ? puzzle.points : Math.floor(puzzle.points * 0.5); Game.state.score += pts; Game.state.puzzleSolved[id] = true; Game.state.puzzlesCompleted++; pEl.classList.add('correct'); rEl.style.display = 'block'; rEl.className = 'puzzle-result correct'; rEl.textContent = `CORRECT! +${pts} points`; playSound('correct'); if (id == 1) { const ring = document.createElement('div'); ring.className = 'biofilm-growth-container'; ring.innerHTML = '<div class="biofilm-growth-ring"></div><div class="biofilm-dot" style="top:40%;left:40%"></div>'; document.querySelector('#room1 .narrative-box')?.appendChild(ring); } const st = pEl.querySelector('.puzzle-status'); if (st) { st.classList.remove('locked'); st.classList.add('solved'); st.innerHTML = '<path d="M4,9 L7,12 L14,5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>'; } const roomNum = getPuzzleRoom(id); updateOrderNumbers(); const roomPuzzles = getRoomPuzzleIds(roomNum); const nextPuzzleId = roomPuzzles[roomPuzzles.indexOf(id) + 1]; if (roomPuzzles.every(p => Game.state.puzzleSolved[p])) setTimeout(() => completeRoom(roomNum), 800); else if (nextPuzzleId) setTimeout(() => showRoomPuzzle(roomNum, nextPuzzleId), 800); } else { pEl.classList.add('incorrect'); rEl.style.display = 'block'; rEl.className = 'puzzle-result incorrect'; rEl.innerHTML = `WRONG - Try again${puzzle.explanation ? `<div style="margin-top:8px;font-size:12px;font-weight:400;color:var(--red);opacity:.85">${puzzle.explanation}</div>` : ''}`; playSound('incorrect'); setTimeout(() => pEl.classList.remove('incorrect'), 500); } updateHUD(); }
